...
Generate a thorough, professional SOP that could be immediately used in a real organization.`; async function y(e) {
        try {
            var t; let { notes: r, title: a, description: n } = await e.json(), o = (t = function (e) { let t = {}; for (let r of e) { let e = r.category || "OTHER"; t[e] || (t[e] = []), t[e].push(r) } return t }(r), Object.entries(t).map(([e, t]) => {
                let r = t.map((e, t) => { let r = `  ${t + 1}. ${e.content}`; return e.relatedTo && (r += ` (Related: ${e.relatedTo})`), e.action && (r += ` - Action: ${e.action}`), r }).join("\n"); return `### ${e.replace(/_/g, " ")}
${r}`
