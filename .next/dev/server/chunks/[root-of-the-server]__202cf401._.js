...
"[project]/app/api/chat/generate-sop/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
// -----------------------------------------------------------------------------
function formatNotesForPrompt(notesByCategory) {
    return Object.entries(notesByCategory).map(([category, categoryNotes])=>{
        const formattedNotes = categoryNotes.map((n, i)=>{
...
// -----------------------------------------------------------------------------
function getMissingSections(structure) {
    const sectionMap = {
    ...
    return Object.entries(sectionMap).filter(([_, present])=>!present).map(([section])=>section);
}
// -----------------------------------------------------------------------------
...
function formatAnalysisForPrompt(analysis) {
    ...
    const issuesList = analysis.improvements.map((i)=>`• [${i.priority}] ${i.category}: ${i.description}`).join('\n');
...
function formatConversationForPrompt(messages) {
    ...
    return relevantMessages.map((msg)=>`**${msg.role.toUpperCase()}:** ${msg.content}`).join('\n\n');
}
// -----------------------------------------------------------------------------
// Build Improvement Prompt
// -----------------------------------------------------------------------------
function buildImprovementPrompt(originalContent, analysis, conversationHistory, notes, title) {
...
// Build Create Prompt
// -----------------------------------------------------------------------------
function buildCreatePrompt(notes, title, description) {
    const notesByCategory = organizeNotes(notes);
    const notesContext = formatNotesForPrompt(notesByCategory);
...
async function POST(req) {
    try {
        ...
        const { mode = 'create', notes = [], title = 'Standard Operating Procedure', description, originalContent, analysis, conversationHistory = [] } = body;
        let systemPrompt;
        let userPrompt;
        if (mode === 'improve' && originalContent) {
            ...
            userPrompt = buildImprovementPrompt(originalContent, analysis || null, conversationHistory, notes, title);
        } else {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__202cf401._.js.map